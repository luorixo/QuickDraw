...
public class UserHomeController {
	...
	/**
	 * After userHome fxml and controller have been constructed this will set the
	 * current user and get needed details. This method will also set the initial
	 * image in the image viewer - either to a waiting image or the corresponding
	 * image from the first word in their played list.
	 */
	...
	private void updateImage() {
		...
		if (wordsSeen.size() == 0) { // set waiting image if user has not drawn anything yet
			...
			file = new File(
					"src/main/resources/users/user" + id + "/images/" + wordsSeen.get(currentWordIndex) + ".png");
		}
	...
	private void updateImageName() {
		...
		if (wordsSeen.size() == 0) { // if no past games then set waiting message
			...
			currentWord = currentWord.substring(0, 1).toUpperCase() + currentWord.substring(1).toLowerCase(); // formats
																												// so
																												// the
																												// first
																												// letter
